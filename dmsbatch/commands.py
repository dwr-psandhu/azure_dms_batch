...
def create_batch_client(config_file):
    """
...
def create_blob_client(config_file):
    ...
    return AzureBlob(config['_STORAGE_ACCOUNT_NAME'], config['_STORAGE_ACCOUNT_KEY'])
...
class AzureBlob:
    ...
    def get_container_sas_url(self,
        ...
        if not blob_permissions:
            blob_permissions = azureblob.BlobPermissions.WRITE
        # Obtain the SAS token for the container.
        sas_token = self.get_container_sas_token(container_name, blob_permissions)
    ...
    def create_sas_token(
            self, container_name, blob_name, permission, expiry=None,
            timeout=None):
        """Create a blob sas token
        ...
        if expiry is None:
            ...
            expiry = datetime.datetime.utcnow() + datetime.timedelta(
                minutes=timeout)
        return self.blob_client.generate_blob_shared_access_signature(
            container_name, blob_name, permission=permission, expiry=expiry)

    def create_sas_url(self, container_name, blob_name, expiry=None, timeout=None, permission=azureblob.BlobPermissions.READ):
    ...
    def upload_blob_and_create_sas(
        ...
        self.blob_client.create_blob_from_path(
            container_name,
            blob_name,
            file_name, max_connections=max_connections)
    ...
    def zip_and_upload(self, container_name, blob_name, dirname, timeout=30):
        ...
        try:
            ...
            if len(blob_name) > 0 and not blob_name.endswith('/'):
                blob_name = blob_name + '/'
            return self.upload_file_to_container(container_name, f'{blob_name}{zipfile}.zip', zipfile + '.zip', timeout=timeout)
        finally:
